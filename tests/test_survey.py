...
def test_probabilities(sim, ztf):
    ...
    visit_prob = sim.syst.visit_prob["ZTF"][0]
    assert abs((visit_prob - 0.5 * duty_cycle) / visit_prob) < 0.2

    # now what happens when we use multiple exposures in a row
    ztf.series_length = 5
    ztf.dead_time = 0.1  # assume fast readout for this test only
    ...
    ztf.apply_detection_statistics(sim.syst, plotting=0)
    # print(
    ...
    assert abs(sim.syst.flare_prob["ZTF"][0] - 1) < 0.01  # close to 100% detection
    # assert abs((sim.syst.visit_prob["ZTF"][0] - 0.5 * duty_cycle) / sim.syst.visit_prob["ZTF"][0]) < 0.2
...
def test_grid_scan(grid):
    ...
    for s in ds.survey.values:
        # run over declinations and then sma
        for i, d in enumerate(ds.declination.values):
            ...
            for j, a in enumerate(ds.semimajor_axis.values):
                ...
                if ev < 100:
                    continue  # do not care about very low volumes, those have negligible probability
                if ev > prev_ev * 1.30:  # allow errors up to 30% increase in volume
                    if not rising_phase:
                        message = (
                            f"Volume increase of {(ev-prev_ev) / prev_ev * 100:.1f}% "
                            f"at sma= {prev_a}->{a} and dec= {d} for {s}. "
                            f"visit prob= {ds.isel(declination=i, semimajor_axis=j-1).sel(survey=s).visit_prob.values} -> "
                            f"{ds.isel(declination=i, semimajor_axis=j).sel(survey=s).visit_prob.values}, "
                            f"volume: {prev_ev} -> {ev}, "
                            f"distance: {ds.isel(declination=i, semimajor_axis=j-1).sel(survey=s).distance.values} -> "
                            f"{ds.isel(declination=i, semimajor_axis=j).sel(survey=s).distance.values}"
                        )
        ...
        # run over sma and then declinations
        for j, a in enumerate(ds.semimajor_axis.values):
            if a < 0.1:  # very small separations are problematic because of eclipses
                continue
            prev_ev = np.inf
