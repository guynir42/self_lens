...
def test_analytic_solution():
    ...
    print(
        f"Source= {source} | occulter= {occulter} | "
...
def test_elliptical_large_source(matrix_large):
    ...
    if not np.all(np.abs(mag_direct - mag_approx) < 0.01):
        ...
        raise ValueError(
            f"At least one point shows a different magnification of {np.max(abs(mag_direct - mag_approx))}"
        )
    ...
    if not np.all(np.abs(mag_direct - mag_approx) < 0.3) or not np.all(
        ...
        if not np.all(np.abs(mag_direct - mag_approx) > 0.01):
            raise ValueError(
                f"At least one point shows a different magnification of {np.min(abs(mag_direct - mag_approx))}"
            )
